...
import MemorialDateTime from '@/app/(pages)/(auth)/(user)/church/book-a-service/[id]/[service]/components/MemorialDateTime'
...
function Memorial({church}) {
  ...
  const { walkinMemorial } = useBook()

  const submitMemorial = (data) => {
        ...
        <form onSubmit={handleSubmit(submitMemorial)}>
          <div className='px-24 flex flex-col md:items-start '>
            <div className='md:flex gap-5 items-center'>
              ...
              <label className='text-end' htmlFor='reserved_by'>RESERVED BY: </label>
              <div className='flex flex-col'>
                <input type="text" id='reserved_by' className='border w-full p-2 outline-none rounded-lg bg-neutral-200' 
                  {...register('reserved_by', {
                    required: `Reservation Name is Required`
                  })}
                />
                {error.reserved_by && <span className='text-red-500 text-sm '>{error.reserved_by.message}</span>}
              </div>
            </div>
          </div>
